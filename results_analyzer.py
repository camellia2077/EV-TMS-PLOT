...
class ResultsAnalyzer:
    ...
    def post_process_data(self):
        ...
        P_inv_in_profile_hist = np.zeros(n_points) # 初始化存储数组
        ...
        self.processed_data['time_data'] = self.raw_results['time_sim'] # 时间序列
        self.processed_data['temperatures'] = self.raw_results['temperatures_data'] # 各部件温度历史
        self.processed_data['ac_power_log'] = self.raw_results['ac_power_log'] # 空调压缩机电功率历史
        # 座舱蒸发器实际制冷功率历史
        self.processed_data['cabin_cool_power_log'] = self.raw_results['cooling_system_logs']['Q_cabin_evap_cooling']
        self.processed_data['speed_profile'] = self.raw_results['speed_profile'] # 车速历史
        self.processed_data['heat_gen_profiles'] = self.raw_results['heat_gen_data'] # 各部件产热/负荷历史
        # 电池相关功率历史
        ...
        self.processed_data['cooling_system_logs'] = self.raw_results['cooling_system_logs'] # 冷却系统详细日志
        ...
        self.processed_data['sim_params_dict'] = {
            ...
            'UA_LTR_max': getattr(self.sp, 'UA_LTR_max', None),
    ...
    def analyze_chiller_transitions(self):
        ...
        found_transitions = False # 标记是否找到任何转变事件
    ...
    def print_temperature_extrema(self, all_temperature_extrema):#打印极值点
        pass
    def print_average_values(self):
        ...
        if q_ltr_to_ambient is not None and len(q_ltr_to_ambient) > 0:
            ...
            print(f"    外置散热器(LTR)实际散热功率: {avg_q_ltr:.2f} W")
        ...
        if q_coolant_from_lcc is not None and len(q_coolant_from_lcc) > 0:
            ...
            print(f"    LCC从制冷剂吸收热量(传递给冷却液): {avg_q_lcc:.2f} W") # 调整描述更准确
        ...
        try:
            ...
            if all_components_present:
                ...
                total_heat_rejection_cooling_effort = q_ltr_arr + q_pt_chiller_arr + q_cabin_evap_arr
                ...
                print("\n  平均总热负荷 vs 总散热系统移除功率 (W):")
                print(f"    总产热/负荷功率: {avg_total_load:.2f} W")
                print(f"    总散热系统移除功率 (LTR吸热+Chiller吸热+CabinEvap吸热): {avg_total_rejection_cooling_effort:.2f} W")
