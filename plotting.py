...
def plot_results(time_data, temperatures, chiller_log, ac_power_log, cabin_cool_power_log,
    ...
    Font sizes for title, axis labels, and tick labels are now controlled by sim_params.
    """
    ...
    axis_label_fs = sim_params.get('axis_label_font_size', 12) # 新增
    tick_label_fs = sim_params.get('tick_label_font_size', 10) # 新增
    title_fs = sim_params.get('title_font_size', 14)           # 新增
    ...
    if 'cabin_cooling_temp_thresholds' in sim_params and 'cabin_cooling_power_levels' in sim_params:
        ...
        for idx, temp_thresh in enumerate(thresholds):
            if levels[idx] > 0 and temp_thresh < 99 :
                 ...
                 plt.axhline(temp_thresh, color='salmon', linestyle=':', alpha=0.4, label=label_text if idx < 2 else None)
    ...
    plt.title(f'部件估算温度 (加速与匀速过程 {sim_params["v_start"]}-{sim_params["v_end"]}km/h, 含空调, COP={cop_value:.2f}, 环境={sim_params["T_ambient"]}°C)', fontsize=title_fs)
    ...
    ax1.set_ylabel('Chiller 状态 (0/1)', fontsize=axis_label_fs)
    ax2.set_ylabel('压缩机功率 (W)', color='cyan', fontsize=axis_label_fs)
    ax1.tick_params(axis='x', labelsize=tick_label_fs)
    ax1.tick_params(axis='y', labelsize=tick_label_fs)
    ax2.tick_params(axis='y', labelcolor='cyan', labelsize=tick_label_fs)
    plt.xlim(left=0, right=sim_params['sim_duration']/60)
    ...
    plt.title('制冷系统状态和总功耗', fontsize=title_fs)
    plt.tight_layout()
    ...
    if 'cabin_cooling_power_levels' in sim_params:
        ...
        plt.ylim(min_power - 0.1 * abs(min_power) if min_power != 0 else -100 , max_power + 0.1 * max_power + 100)
    else:
        plt.ylim(bottom=0)
