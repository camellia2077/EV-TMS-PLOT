...
def F_aero_func(v_kmh, T_amb):
    """计算车辆的空气阻力。
    参数:
    v_kmh: 车速 (单位: 千米/小时 km/h)。
    T_amb : 环境空气温度 (单位: 摄氏度 °C)，用于计算空气密度。
    返回:
    float: 空气阻力 (单位: 牛顿 N)。
    """
    rho = rho_air_func(T_amb) # 计算当前环境温度下的空气密度(单位: kg/m^3)
    Cd = 0.22# 空气阻力系数 (风阻系数,无量纲)，取决于车辆外形设计
    a = 3.00# 车辆的迎风面积 (单位: 平方米 m^2)
    vmps = v_kmh / 3.6# 将车速从 千米/小时 (km/h) 转换为 米/秒 (m/s)
...
def P_wheel_func(v_kmh, m, T_amb):
    ...
    v_kmh : 车速 (单位: 千米/小时 km/h)。
    m : 车辆总质量 (单位: 千克 kg)。
    T_amb : 环境空气温度 (单位: 摄氏度 °C)。
    返回:
    float: 车轮处的驱动功率需求 (单位: 瓦特 W)。
    """
    force_roll = F_roll_func(m) #计算滚动阻力(单位: N)
    force_aero = F_aero_func(v_kmh, T_amb)#计算空气阻力 (单位: N)
...
def P_motor_func(p_wheel, eta_m):
    """
    计算电机输入功率 (即电机控制器或逆变器输出给电机的功率)。
    参数:
    p_wheel (float): 车轮处的驱动功率需求 (单位: 瓦特 W)。
    eta_m (float): 电机的效率。
    返回:
    float: 电机的输入功率 (单位: 瓦特 W)。
    """
...
def Q_mot_func(p_motor_in, eta_m):
    ...
    计算电机的产热功率。
    参数:
    p_motor_in (float): 电机的输入功率 (单位: 瓦特 W)。
    eta_m (float): 电机的效率 (0到1之间的数值)。
    返回:
    float: 电机的产热功率 (单位: 瓦特 W)。
    """
...
def Q_inv_func(p_motor_in, eta_inv):
    ...
    计算逆变器 (或电机控制器) 的产热功率。
    参数:
    p_motor_in (float): 电机的输入功率 (也是逆变器的大致输出功率) (单位: 瓦特 W)。
    eta_inv (float): 逆变器的效率 (0到1之间的数值)。
    返回:
    逆变器的产热功率 (单位: 瓦特 W)。
    """
...
def Q_batt_func(p_elec_total, u_batt, r_int):
    ...
    计算电池的产热功率。
    参数:
    p_elec_total : 车辆总的电功率需求 (近似等于电池输出功率) (单位: 瓦特 W)。
    u_batt: 电池的端电压 (单位:  V)。
    r_int: 电池的等效内阻 (单位: Ω)。
    返回:
    float: 电池的产热功率 (单位: W)。
    """
    I_batt = p_elec_total / u_batt # 计算电池电流 I_batt = P_elec_total / U_batt(单位: 安培 A)
